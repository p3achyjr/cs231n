...
def sample_noise(batch_size, dim, seed=None):
    ...
    return (2 * torch.rand(batch_size * dim) - 1).reshape((batch_size, dim))
...
def generator_loss(logits_fake):
    ...
    labels = torch.ones(N).type(dtype)
...
def get_optimizer(model):
    ...
    optimizer = optim.Adam(model.parameters(), lr=1e-3, betas=(.5, .999))
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    real_loss = torch.mean(.5 * (scores_real - 1) ** 2)
    fake_loss = torch.mean(.5 * scores_fake ** 2)
    loss = real_loss + fake_loss
...
def ls_generator_loss(scores_fake):
    ...
    loss = torch.mean(.5 * (scores_fake - 1) ** 2)
