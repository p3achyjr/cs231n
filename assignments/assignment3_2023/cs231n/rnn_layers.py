...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    h = prev_h.dot(Wh) # (N, H)
    next_h_pre = xh + h + b
    next_h = np.tanh(next_h_pre)
    cache = (next_h_pre, h, xh, x, prev_h, Wx, Wh, b)
...
def rnn_step_backward(dnext_h, cache):
    ...
    next_h_pre, h, xh, x, prev_h, Wx, Wh, b = cache
    dnext_h_pre = (1 - np.tanh(next_h_pre) ** 2) * dnext_h # (N, H)
    dh = dnext_h_pre # (N, H)
    dxh = dnext_h_pre # (N, H)

    db = np.sum(dnext_h_pre, axis=0)
    dWh = prev_h.T.dot(dh)# (H, N) * (N, H)
    dWx = x.T.dot(dxh) # (D, N) * (N, H)
    dprev_h = dh.dot(Wh.T)
    dx = dxh.dot(Wx.T)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    ht_1 = h0
    for xt in x:
      ht, cache_t = rnn_step_forward(xt, ht_1, Wx, Wh, b)
      h.append(ht)
      cache_ts.append(cache_t)
      ht_1 = ht

    h = np.asarray(h)
    h = h.transpose((1, 0, 2)) # (T, N, D) -> (N, T, D)
    cache = (x, h0, h.transpose((1, 0, 2)), Wx, Wh, b, cache_ts)
...
def rnn_backward(dh, cache):
    ...
    # dW_t = dh_t * (dh_t / dh_t-1 * dh_t-1 / dh_t-2 ... dh_2 / dh_1) + dh_1 / dW
    # db_t = dh_t * (dh_t / dh_t-1 * dh_t-1 / dh_t-2 ... dh_2 / dh_1) + dh_1 / db
    def elem_mult(dhs, dh_partials, dW):
      dW_new = np.zeros_like(dW)
      for dh, dh_partial in zip(dhs, dh_partials):
        dW_new += dh * dh_partial * dW

      return dW_new

    # def elem_mult(dhs, dW):
    #   dW_new = np.zeros_like(dW)
    #   for dh in dhs:
    #     dW_new += dh * dW
    ...
    dh = dh.transpose((1, 0, 2)) # (N, T, H) -> (T, N, H)
    x, h0, h, Wx, Wh, b, cache_ts = cache
    x0 = x[0]
    
    dh_t = 0
    dts = []
    dWx, dWh, db = 0, 0, 0
    for i, cache_t in reversed(list(enumerate(cache_ts))):
      dx, dprev_h, dWx_t, dWh_t, db_t = rnn_step_backward(dh_t + dh[i], cache_t)
      dWx += dWx_t
      dWh += dWh_t
      db += db_t

      dts.append({'dx': dx, 'dprev_h': dprev_h, 'dWx': dWx_t, 'dWh': dWh_t, 'db': db_t})
      dh_t = dprev_h

    dts.reverse()

    dx = np.asarray([dt['dx'] for dt in dts])
