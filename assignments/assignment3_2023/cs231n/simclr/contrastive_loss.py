...
def sim(z_i, z_j):
    ...
    norm_dot_product = z_i.dot(z_j) / (torch.linalg.norm(z_i) *
                                       torch.linalg.norm(z_j))
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        esim_sum = 0
        for i in range(2 * N):
          esim = torch.exp(sim(z_k, out[i]) / tau)
          esim_sum += esim

        esimk = torch.exp(sim(z_k, z_k_N) / tau)
        esimkN = torch.exp(sim(z_k_N, z_k) / tau)

        esim_sum_k = esim_sum - torch.exp(sim(z_k, z_k) / tau)
        esim_sum_kN = esim_sum - torch.exp(sim(z_k_N, z_k_N) / tau)

        total_loss += -torch.log(esimk / esim_sum_k)
        total_loss += -torch.log(esimkN / esim_sum_kN)
...
def sim_positive_pairs(out_left, out_right):
    ...
    pos_pairs = pos_pairs / (torch.linalg.norm(out_left, dim=1) *
...
def compute_sim_matrix(out):
    ...
    norms = torch.linalg.norm(out, dim=1).unsqueeze(dim=-1)
    norm_matrix = torch.matmul(norms, norms.T)
    sim_matrix = torch.matmul(out, out.T) / norm_matrix
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    simL = sim_positive_pairs(out_left, out_right)
    simR = sim_positive_pairs(out_right, out_left)
    sim = torch.concat([simL, simR], dim=0)
    ...
    numerator = torch.exp(sim / tau).squeeze()
